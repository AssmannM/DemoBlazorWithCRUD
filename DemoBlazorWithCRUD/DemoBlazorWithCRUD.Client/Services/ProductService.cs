...
namespace DemoBlazorWithCRUD.Client.Services
{
	public class ProductService : IProductReposistory
	{
		...
		{
			var product = await httpClient.DeleteAsync($"api/Product/Single-Product{productId}");
			var response = await product.Content.ReadFromJsonAsync<Product>();
